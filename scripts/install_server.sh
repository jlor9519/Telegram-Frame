...
display_pi_url="${display_pi_url%/}"  # strip trailing slash
...
set_yaml_value inkypi.update_method string "http_update_now"
...
echo
echo "Note: Settings that require InkyPi to restart (orientation, slideshow interval,"
echo "sleep schedule) require the display Pi to be reachable and InkyPi to reload its config."
echo "Use /status in Telegram to check if the display Pi is reachable."
